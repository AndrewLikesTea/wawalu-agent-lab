...
def propose_directive_plan(manager_prompt: str, product: str, recent_titles: list[str],
    ...
    guidance = (
        ...
        "Every task must name its consuming surface: one acceptance criterion has to say which "
        "existing page, view, or entry point imports, renders, or invokes the new code, and require "
        "that wiring to ship in the same pull request. Review rejects a diff that only adds modules, "
        "fixtures, or tests nothing in the product imports, so never author a task whose criteria "
        "can be satisfied without touching an existing product surface. "
