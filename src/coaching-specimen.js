...
export const SPECIMEN_CASES = Object.freeze([
  Object.freeze({
    ...
    purpose: "The ordinary case. Read it top to bottom: the answer, the scale it sits on, one move, and nothing else demanding attention.",
  }),
  Object.freeze({
    id: "graded-strong",
    sample: "well-formed-request",
    label: "Graded — a request that states its context",
    purpose: "The top of the scale, where there is no band above and the one move is a credit rather than a fix. The layout must not look broken when the news is good.",
  }),
  Object.freeze({
    id: "loading",
    sample: null,
    ...
    label: "Grading",
    purpose: "What a caller paints while the rubric reads. Every region keeps its label and its place, so nothing moves under the reader when the figures land.",
  }),
  Object.freeze({
    id: "empty",
    sample: "empty-box",
    label: "Nothing to grade — an empty box",
    purpose: "The state a first visit produces most often. It has to be a state with a next action in it, not a blank panel.",
  }),
  Object.freeze({
    id: "invalid",
    sample: "long-transcript",
    label: "Not accepted — a whole transcript",
    purpose: "A rejected input still gets a benchmark: the ceiling it hit, as a count, so a reader knows what to trim to.",
  }),
  Object.freeze({
    id: "unsupported",
    sample: "pasted-log-only",
    label: "Nothing the rubric can read — a pasted log",
    purpose: "Accepted text the rubric could not read a request out of. The distinction from “not accepted” has to survive a skim, which is why the words differ and not only the tint.",
  }),
  Object.freeze({
    id: "implausible",
    sample: "underspecified-request",
    ...
    purpose: "Not reachable from the shipped engine. The letter is withheld rather than printed with a caveat, each bad figure is named beside its bound, and the evidence below stays readable.",
  }),
]);
