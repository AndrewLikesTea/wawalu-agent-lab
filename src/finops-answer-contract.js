...
// figures became one, seven regions became one answer with a supporting layer.
// Every one of those changes was a rewrite of the words around a number, and a
// rewrite is exactly when a number quietly stops meaning what it meant. Nothing
// on the page could tell a reader that the surviving answer is still the answer
// the arithmetic produces.
//
// So four dimensions of the consolidated answer are ATTESTED here, computed from
// the record `getRecoverableSpend` already published, and pinned value-for-value
// in tests/fixtures/finops-consolidated-answer-attestation.json:
//
//   headline    the canonical monthly recoverable figure
//   confidence  the band it is graded at
//   provenance  how many of its operands the export DECLARED against how many
//               this page DERIVED
//   coverage    departments scored against departments in scope
//
// NO NUMBER IS INVENTED HERE AND NO SECOND ARITHMETIC PATH EXISTS. Every value
// below is a field of the record above or a count of the operand table below;
// nothing re-sums a department and nothing re-grades a confidence.
//
// WHY DECLARED-AGAINST-DERIVED IS THE PROVENANCE MEASURE. A figure the export
// states survives a challenge from the person who exported it; a figure this
// page worked out does not, and has to be defended on its rule instead. The
// split is therefore the one provenance fact a director disputing this number
// needs first, and it is CHECKED rather than labelled: an operand counts as
// declared only when the analysis envelope states that exact value itself, which
// `attestationOperands` verifies operand by operand on every call. Relabelling
// a derived figure "declared" is not possible without moving the envelope.
// ---------------------------------------------------------------------------
 ...
 *
 * `value` is what the answer publishes. `stated` is what the ANALYSIS ENVELOPE
 * says on its own — `undefined` when the envelope says nothing, which is what
 * makes the operand derived. Neither reads the page.
 */
export const ATTESTATION_OPERANDS = Object.freeze([
  Object.freeze({
    key: "period",
    value: (record) => record.periodLabel,
    stated: (dataset) => (filled(dataset?.period) ? dataset.period.trim() : undefined),
    assumption: "The reporting window is the export's own and is never inferred: a recoverable"
      + " figure with no period attached cannot be checked against an invoice.",
  }),
  Object.freeze({
    key: "departmentsInScope",
    value: (record) => record.totalDepartments,
    stated: (dataset) => departmentRows(dataset).length,
    assumption: "Scope is every department row the export published, scored or not. Counting"
      + " rows states nothing the envelope did not, so this is a declared operand.",
  }),
  Object.freeze({
    key: "analyzedSpendUsd",
    value: (_record, dataset) => (Number.isFinite(dataset?.spendUsd) ? dataset.spendUsd : null),
    stated: (dataset) => (Number.isFinite(dataset?.spendUsd) ? dataset.spendUsd : undefined),
    assumption: "The denominator the headline is a share of is the envelope's own analyzed"
      + " total. This page does not re-sum it, so it cannot disagree with the export.",
  }),
  Object.freeze({
    key: "monthlyRecoverableUsd",
    value: (record) => record.monthly,
    stated: (dataset) =>
      (Number.isFinite(dataset?.recoverableUsd) ? roundHalfUp(dataset.recoverableUsd) : undefined),
    assumption: "The headline is declared only while the envelope's own recoverable total and"
      + " the sum over scored rows agree. They disagree and it becomes derived, which is"
      + " the reconciliation #1502 landed, measured rather than asserted.",
  }),
  Object.freeze({
    key: "confidenceBand",
    value: (record) => record.confidence.level,
    stated: (dataset) => recoverableConfidenceLevel(dataset) ?? undefined,
    assumption: "The band is the analysis's existing confidence signal, read through the"
      + " published cut points. No second confidence scale is invented for this answer.",
  }),
  Object.freeze({
    key: "scoredDepartments",
    value: (record) => record.scoredDepartments,
    stated: () => undefined,
    assumption: "Derived: the envelope publishes no scored count, so this page applies the"
      + " completed-score test row by row. It is the operand a reader is most owed a rule for.",
  }),
  Object.freeze({
    key: "annualRecoverableUsd",
    value: (record) => record.annualised,
    stated: () => undefined,
    assumption: "Derived: the monthly figure multiplied by 12. No annual constant is stored,"
      + " because a stored one is the figure that drifted away from the departments.",
  }),
]);

export const ATTESTATION_ORIGIN = Object.freeze({ declared: "declared", derived: "derived" });

/** Each operand's origin, decided by comparing the published value with what the
 *  envelope states — never by the label written beside it. */
export function attestationOperands(dataset, record = getRecoverableSpend(dataset)) {
  return ATTESTATION_OPERANDS.map((operand) => {
    const value = operand.value(record, dataset);
    const stated = operand.stated(dataset);
    const declared = stated !== undefined && Object.is(stated, value);
    return Object.freeze({
      key: operand.key, value, stated: stated ?? null, assumption: operand.assumption,
      origin: declared ? ATTESTATION_ORIGIN.declared : ATTESTATION_ORIGIN.derived,
    });
  });
}

/**
 * The four attested dimensions, and the one sentence the page states them in.
 *
 ...
 *   that already hold one so the figure is read once and not recomputed.
 * @returns a frozen `finops-recoverable-attestation/1.0.0` record.
 */
export function recoverableAttestation(dataset, record = getRecoverableSpend(dataset)) {
  const operands = attestationOperands(dataset, record);
  const count = (origin) => operands.filter((operand) => operand.origin === origin).length;
  const declared = count(ATTESTATION_ORIGIN.declared);
  const derived = count(ATTESTATION_ORIGIN.derived);
  const headline = record.monthlyDisplay ?? "not stated";
  const confidence = record.confidence.level ?? "not graded";
  const dimensions = Object.freeze({
    headline, confidence,
    provenance: Object.freeze({ declared, derived, total: operands.length }),
    coverage: Object.freeze({
      scored: record.scoredDepartments, inScope: record.totalDepartments,
    }),
  });
  return Object.freeze({
    version: RECOVERABLE_ATTESTATION, dimensions,
    operands: Object.freeze(operands),
    // THE HEADLINE IS NOT RESTATED HERE. This region states its one recoverable
    // figure once and its projection once; a third money string in it would be a
    // figure competing with the answer again, which is the defect #1496 and
