...
/**
 * A FIGURE stays a figure. A derived value that is a bare number may only be
 * corrected to a bare number: a reader who types a word over a total leaves the
 * arithmetic beside it holding something it cannot divide, and refusing is the
 * only answer that does not corrupt the state. A derived value that is a
 * SENTENCE ("33% of analyzed AI spend is recoverable") is prose and takes prose.
 */
export const acceptsCorrection = (unit, field, value) => {
  const accepted = acceptedCorrection(value);
  if (!accepted) return false;
  return isNumericValue(unit?.derived?.[field]) ? isNumericValue(accepted) : true;
};
...
export function applyCorrection(state, unitId, field, value) {
  const unit = CORRECTION_FIELDS.includes(field) ? correctionUnit(state, unitId) : null;
  if (!unit || !acceptsCorrection(unit, field, value)) return closeEditor(state);
  const accepted = acceptedCorrection(value);
  return closeEditor(withUnit(state, unitId, (held) => Object.freeze({
    ...held,
    corrected: Object.freeze({ ...held.corrected, [field]: accepted }),
  })));
}
...
/** What the polite region says once, after a correction, a revert, or a refusal. */
...
/** What a refused value is told, in the same region, naming why. Silence here
 *  would read as a correction that landed. */
export const refusalAnnouncement = (state, unitId, field) => {
  const unit = correctionUnit(state, unitId);
  const noun = FIELD_WORDS[field]?.noun ?? "value";
  return `That ${noun} was not applied: ${unitValue(unit, "name")} takes a number, `
    + `and the derived value ${unit?.derived?.[field] ?? ""} is unchanged.`;
};
...
function editorParts(doc, unit, field, handlers) {
  ...
  const name = unitValue(unit, "name");
  const numeric = isNumericValue(unit.derived[field]);
  const inputId = correctionControlId(unit.id, field, "input");
  const label = doc.createElement("label");
  label.className = "visually-hidden";
  label.setAttribute("for", inputId);
  label.textContent = numeric
    ? `Corrected ${noun} for ${name}, a number` : `Corrected ${noun} for ${name}`;
...
export function mountFigureCorrections(doc, { entries = EXAMPLE_FIGURE_SOURCES } = {}) {
  ...
  const handlers = Object.freeze({
    ...
    commit(unitId, field, value) {
      const unit = correctionUnit(state, unitId);
      const accepted = acceptsCorrection(unit, field, value);
      // An empty commit is a cancel and says nothing; a value this page cannot
      // hold is refused out loud, with the derived value left standing.
      const refused = !accepted && Boolean(acceptedCorrection(value));
      ...
      else if (refused) announce(refusalAnnouncement(state, unitId, field));
    },
    cancel(unitId, field) {
