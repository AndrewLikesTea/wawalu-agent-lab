...
// `rateCardHedge` and `rateCardNextStep` are called from here and nowhere that
// writes to a document, so a second tier word cannot reach the DOM: the view
// layer has no other function to call.
//
// ---------------------------------------------------------------------------
...
// invariant itself is asserted over the bundled example and the real dataset in
// tests/finops-recoverable-readiness.test.js.
//
 ...
 * published in cents, so anything larger is a row this contract cannot see, and
 * anything smaller is the last binary digit of a decimal sum.
 */
 ...
 * tests/finops-recoverable-readiness.test.js asserts a known department of the
 * real dataset reads `scored === true`, so a divergence fails loudly instead.
 */
...
export function resolveRecoverableReadiness(signals) {
  ...
  return publish(complete ? READINESS_STATE.complete : READINESS_STATE.partial,
    ...
    Math.round(eligibleSpend * 100) / 100, Math.round(total * 100) / 100);
}
 ...
 * from, and how many asks are outstanding — the asks themselves are the
 * `blockers` records, rendered as their own list by the view.
 */
