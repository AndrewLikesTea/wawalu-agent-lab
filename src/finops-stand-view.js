...
export function applyStandHeadline(doc, headline, { announce = true, trust = "" } = {}) {
  ...
  // `trust` is the rung the reader's own brief now stands on, handed in by the
  // caller that just read the file rather than re-derived here. It rides this
  // one sentence; the ladder paints no announcer of its own.
