...
export async function handleLeadRequest(request, {
  ...
  const isFollowUp = keys.length === 2 && keys.includes("email") && keys.includes("interest");
  const isPurposeLead = keys.length === 2 && keys.includes("email") && keys.includes("purpose");
  if (!isObject || (!isFollowUp && !isPurposeLead)) {
    return json({ error: { code: "invalid_request", message: "Body contains unsupported fields.", request_id: requestId } }, 400, requestId);
  }
  const purpose = isFollowUp ? "follow_up" : input.purpose;
  ...
  if (isFollowUp && (typeof input.interest !== "string" || input.interest.trim().length > MAX_INTEREST_LENGTH)) {
  ...
  const interest = isFollowUp ? (input.interest.trim() || null) : null;
