 ...
 * "How we know this" pattern: the same element, the same summary words, the same
 * classes as the disclosure under the recoverable headline. The parts are #1287's
 * four steps rather than the headline's three, because what is being explained is
 * an arithmetic derivation and not a provenance — same control, same shape, and
 * the step order comes from `PLAN_TOTAL_STEPS` so it cannot drift per surface.
 *
...
function moveDerivation(doc, entry, basisText) {
  ...
  state.append(shape, doc.createTextNode(" How we know this"));
