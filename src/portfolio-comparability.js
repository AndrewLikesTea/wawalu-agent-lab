...
// is not a league table, and the question here is whether the set is combinable,
// not who is worst. It opens no adapter, reads no credential, and receives no
// prompt or customer text: the evaluation unit is a count of delivered requests
// and a provenance *label*. The contract is shaped so a later aggregation layer
// or a live adapter can consume it unchanged; neither is implemented here.
//
...
//
// SCHEMA VERSION. `wawalu.finops.portfolio-comparability/1.0`. Bump when a
// field, a state, a remediation tier, or a metric definition changes meaning: a
// stored verdict is only interpretable by the version that produced it.
...
export const PORTFOLIO_COMPARABILITY_VERSION = "1.0";
 ...
 *            providers, no valid period, no portfolio currency. No record
 *            supplied later fixes that; the question has to be re-asked.
 */
 ...
 *   deliveryCount, provenance: { label } }`.
...
export function evaluatePortfolioComparability(portfolio) {
  const input = portfolio && typeof portfolio === "object" ? portfolio : {};
