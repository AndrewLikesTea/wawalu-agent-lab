...
export const PROFILE_EMPTY_COPY = {
  // The hero description: states the situation, once. It names the persona
  // rather than the page, because People is never empty — one demo persona is.
  summary: "No image posts from this persona yet.",
...
function renderError(container, onRetry) {
  ...
  // Named for what is missing here — image posts — and for the feed they come
  // from, so this panel cannot be mistaken for the one Social renders.
  failed.append(el("p", "empty-title", "Image posts could not be loaded."));
  failed.append(el("p", undefined, "The connection to the Social feed failed. Nothing was lost — try again."));
