...
const BUNDLED_EXAMPLE_FAILURE = "The example prompt could not be loaded. You can still paste and grade your own prompt.";
...
function initBundledRegion(doc, { bodyId, sectionId, titleId, statusId, loadingClass, loader, failureCopy, retryLabel }) {
  ...
  const run = () => {
    ...
    Promise.resolve(request).then(() => {
      ...
      // "Loading the example prompt." standing under a state attribute that
...
export function initPromptCoaching(doc = globalThis.document, {
  ...
  initBundledRegion(doc, {
    ...
    retryLabel: "Retry the example prompt",
  });
