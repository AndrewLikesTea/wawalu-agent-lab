...
export function renderProviderExportProjection(document, projection, options) {
  if (!projection?.ok) return false;
  return renderOwnDataEvidencePreflight(document, providerExportPreflight(projection, options));
}
