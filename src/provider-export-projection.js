...
import {
  EVIDENCE_PREFLIGHT_OUTCOME, EVIDENCE_SOURCE, OWN_DATA_PREFLIGHT_BOUNDARY,
  OWN_DATA_PREFLIGHT_QUESTION, REQUIRED_SPEND_COVERAGE_BENCHMARK,
 ...
 */
export function providerExportPreflight(projection, { selectedExports = 1 } = {}) {
  ...
  return Object.freeze({
    ...
    source: EVIDENCE_SOURCE.SELECTED_EXPORT.id,
    // One region answers about one export. A reader who selects several would
    // otherwise read the newest file's coverage as if it graded all of them, so
    // the count travels with the caption rather than being left to be assumed.
    sourceLabel: Number.isInteger(selectedExports) && selectedExports > 1
      ? `Evidence preflight · newest of ${selectedExports} selected provider exports`
      : EVIDENCE_SOURCE.SELECTED_EXPORT.label,
    sufficient,
