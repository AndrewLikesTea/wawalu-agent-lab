...
export function initReleasesPage(root = document, storage = localStorage, options = {}) {
  ...
  // The decision half of the same distinction, held the same way. A brief now
  // quotes each linked decision's owner and context, so a seeded decision has
  // to say it is invented there too: a release the visitor recorded may link
  // one, and that release's own provenance line says nothing about the
  // decisions it carried.
