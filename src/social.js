...
function renderPostCard(post, { index }) {
  ...
  // The post's own words, in one shape whether or not a picture follows them: an
  // image post's text used to be a 17px figcaption and a text post's a 15px
  // paragraph, which made the same field two different things to read. It is now
  // .post-body on both paths. `post-caption` carries no styles and never should
  // — it is the selector that says "the body of an image post", which is how the
  // tests reach this paragraph without matching a text post's.
  const image = normalizeImage(post.image);
  const body = el("p", image ? "post-body post-caption" : "post-body", post.body);
  ...
  if (image) {
    ...
    // The post's words precede the image. The figure caption is therefore the
    // image description, not a second copy of the post body.
  ...
  const time = el("time", "post-date post-card-date", formatDateTime(post.createdAt));
