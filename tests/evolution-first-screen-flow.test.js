...
test("the answer sentence is the dominant value in its own block, in both rungs", async () => {
  ...
  // the region actually ships with. Six evidence figures used to outrank it.
...
test("the journey's live region announces from outside the disclosure that folds it", async () => {
  ...
  try {
    ...
    // It ships polite and `silenceEchoedRegions` takes the voice away at boot,
    // because the answer's own announcer says the same thing once. That is why
    // the structural fix matters rather than being moot: the day this region is
    // re-voiced, a shut disclosure would swallow it silently.
    assert.equal(live.getAttribute("aria-live"), "off",
      "the journey echo is announcing a second time over the answer's announcer");
    ...
    // And the announcer that DOES carry this content's voice is outside every
    // disclosure too, and still polite: the echo was silenced, not the message.
...
test("every evidence disclosure on the first screen opens on one press", async () => {
  ...
  try {
    const { document } = page;
    const summaries = ["disclosure-next-step", "disclosure-journey",
      "finops-first-run-how-we-know", "finops-first-run-method"]
      .map((id) => ({ id, details: byId(document, id) }));

    for (const { id, details } of summaries) {
