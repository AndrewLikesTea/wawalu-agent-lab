...
/**
 * The text of every `max-width:640px` block in a stylesheet, concatenated.
 *
 * Matching a selector against the whole file would pass on the desktop rule of
 * the same name, so the narrow-layout claim is made against the narrow blocks
 * alone. Brace depth, not a bounded `[\s\S]{0,n}` window, decides where a block
 * ends — a window long enough to reach the rule is also long enough to run past
 * the closing brace and prove nothing.
 */
function narrowLayoutRules(css) {
  const blocks = [];
  ...
  for (let at = css.indexOf(marker); at !== -1; at = css.indexOf(marker, at + 1)) {
    ...
    for (let i = css.indexOf("{", at); i < css.length; i += 1) {
      if (css[i] === "{") depth += 1;
      else if (css[i] === "}" && (depth -= 1) === 0) { blocks.push(css.slice(at, i + 1)); break; }
    }
  }
  return blocks.join("\n");
}
...
test("a cold visitor can reach the file choice and meets no pre-import trust gate", async () => {
  ...
  try {
    ...
    // And the keyboard reaches it: Tab to the button a cold visitor meets first,
    // Enter, and focus is on the control that reads the file. Asserted last,
    // because it is the one step here that moves the page.
...
// The focus ring and the narrow layout are decided in the stylesheet, so they
// are asserted there — no page is booted for this one. Both are claims the
// harness genuinely cannot make: it paints nothing, so a synthetic focus event
// proves no outline, and it models no viewport, so a `matchMedia` shim would
// only assert itself (no module on this page reads `matchMedia` or `innerWidth`
// — the narrow layout is pure CSS).
test("the import controls keep a visible focus ring and a narrow-screen stacking rule", () => {
  assert.match(STYLES,
    /(^|[,\s{}])input:focus-visible[^{]*\{[^}]*outline:3px solid var\(--focus-ring\)/m,
    "an input element must earn a visible outline on keyboard focus");
  assert.doesNotMatch(EVOLUTION_CSS,
    /#local-finops-files[^{]*:focus[^{]*\{[^}]*outline\s*:\s*(?:none|0)/,
    "the import control must not opt out of the global focus ring");
  assert.doesNotMatch(EVOLUTION_CSS,
    /#activate-local-export[^{]*:focus[^{]*\{[^}]*outline\s*:\s*(?:none|0)/,
    "the activation button must not opt out of the global focus ring either");

  const narrow = narrowLayoutRules(EVOLUTION_CSS);
  assert.notEqual(narrow, "", "evolution.css must ship a narrow-screen media query");
  assert.match(narrow,
    /\.local-import-heading,\.local-import-controls,\.local-result-heading \{[^}]*flex-direction:column/,
    "the import and result headings must stack rather than overflow on a narrow screen");
  assert.match(narrow, /\.local-import-progress \{[^}]*flex-direction:column/,
    "the in-flight import progress row must stack too, or it overflows mid-read");
});
