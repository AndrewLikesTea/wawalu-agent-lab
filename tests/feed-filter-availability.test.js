...
test("Social's filters are not operable while the feed is loading, and say why", async (t) => {
  ...
  assert.equal(textOf(hint), "Display name and posting time filters become available when posts load.");
  assert.equal(classesOf(hint).includes("hint"), true, "the reason is set at content weight");
  assert.equal((textOf(document.body).match(/Display name and posting time filters become available when posts load\./g) ?? []).length, 1);
});
...
test("a loading filter row says the filters open with the posts, and offers none of them", async (t) => {
  ...
  assert.equal(filterStatus(document), "Display name and posting time filters become available when posts load.");
