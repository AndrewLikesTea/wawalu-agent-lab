...
test("Social says one thing while it loads, and the other three lines are not on the page", async (t) => {
  ...
  assert.equal((body.match(/Existing posts are still loading\. Select Publish a post to open the composer\./g) ?? []).length, 1);
...
test("Social's promise about new posts is said only where there is a feed for them", async (t) => {
  ...
  assert.equal((textOf(document.body).match(/Existing posts are still loading\. Select Publish a post to open the composer\./g) ?? []).length, 1);
  ...
  assert.equal(promiseCount(document, promise), 0);
  assert.equal(document.querySelectorAll(".feed-connection").length, 0);
  assert.equal(textOf(document.querySelector("#feed-state").querySelector(".state-title")), "Existing posts are still loading. Select Publish a post to open the composer.");
