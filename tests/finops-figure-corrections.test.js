...
test("an empty or over-long commit is a cancel, and a figure keeps its type", () => {
  const start = createCorrectionState();
  const before = correctionCounts(start);
  for (const refused of ["", "   ", "x".repeat(161)]) {
    assert.deepEqual(correctionCounts(applyCorrection(start, LEAD, "name", refused)), before,
      `${refused.length} characters wrote a correction it should have refused`);
  }

  // A derived value that IS a bare number takes a number and nothing else, and a
  // refusal leaves the derivation standing rather than an empty cell.
  const numeric = createCorrectionState([
    { id: "score", qualifies: "Confidence score", value: "85", source: "derived" },
  ]);
  assert.equal(correctionCounts(applyCorrection(numeric, "score", "figure", "not a number"))
    .correctedCount, 0, "a word typed over a figure must be refused");
  assert.equal(headlineText(applyCorrection(numeric, "score", "figure", "not a number")),
    headlineText(numeric), "a refused figure must leave the brief exactly as it was");
  assert.equal(correctionCounts(applyCorrection(numeric, "score", "figure", "72"))
    .correctedCount, 1, "a number typed over a figure is a correction");
});
...
test("a correction carrying markup renders as text and creates no element", async () => {
  const { page, document } = await mounted();
  try {
    const injected = "<img src=x onerror=alert(1)>Cloud";
    document.getElementById(correctionControlId(LEAD, "name", "edit")).click();
    ...
    // The cell's own text IS the string, not a prefix of something parsed.
    const value = walk(document.getElementById(CORRECTION_IDS.rows))
      .find((node) => node.classList?.contains?.("figure-corrections-value"));
    assert.equal(value.textContent, injected);
    // And nothing became markup: no injected element anywhere on the page, and
    // no element carrying the injected handler as an attribute. Counts, never a
    // null comparison — the harness walks the whole page for the latter.
...
test("a long correction wraps in its cell and the controls take the site's own ring", async () => {
  const evolution = await readFile(new URL("../src/evolution.css", import.meta.url), "utf8");
  const site = await readFile(new URL("../src/styles.css", import.meta.url), "utf8");
  const rule = (selector) => {
    const found = evolution.match(new RegExp(
      `(?:^|[,}])\\s*${selector.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}\\s*\\{([^}]*)\\}`, "m"));
    assert.ok(found, `no rule for ${selector}`);
    return found[1];
  };

  // A pasted 160-character name must wrap inside the cell rather than push the
  // row past the viewport, and the table itself scrolls rather than squeezing.
  assert.match(rule(".figure-corrections-value"), /overflow-wrap:anywhere/);
  assert.match(rule(".figure-corrections-headline, .figure-corrections-confidence"),
    /overflow-wrap:anywhere/);
  assert.match(rule(".figure-corrections-table"), /overflow-x:auto/);
  assert.match(rule(".figure-corrections-input"), /max-width:\d/);

  // No literal colour: every value the block draws is a declared token.
  for (const selector of [".figure-corrections-confidence", ".figure-corrections-caption",
    ".figure-corrections-table th, .figure-corrections-table td"]) {
    ...
    for (const token of body.matchAll(/var\(--([\w-]+)/g)) {
      assert.ok(new RegExp(`--${token[1]}\\s*:`).test(evolution + site),
        `--${token[1]} is used here and declared nowhere`);
    }
  }

  // The focus ring is INHERITED, not redeclared. A second treatment for these
  // controls is the one place the site's ring could drift away from itself.
  assert.match(site, /input:focus-visible[^{]*button:focus-visible[^{]*\{[^}]*var\(--focus-ring\)/);
  assert.doesNotMatch(evolution, /\.figure-corrections-\w+:focus-visible/);
});
