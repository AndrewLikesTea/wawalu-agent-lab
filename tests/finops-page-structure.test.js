...
// ---------------------------------------------------------------------------
// 1b. THE COLLAPSE (#1498): one canonical answer, one supporting-detail layer.
//
// The page answered the recoverable-spend question in more than one top-level
// region, each restating it with a slightly different framing, and a leader had
// to decide which one was authoritative before they could read any of them.
// #1498 folded the readiness region INSIDE the canonical answer, into one
// labelled supporting-detail group.
//
// These assertions are the ones that make the fold a merge rather than an
// amputation: the region is still on the page, still complete, still declared,
// still named — and the capabilities the page offers are still reachable.
// ---------------------------------------------------------------------------

/** True when `node` sits inside the answer's supporting-detail group. */
const inSupportLayer = (node) => {
  for (let up = node; up; up = up.parentNode) if (up.id === "finops-answer-support") return true;
  return false;
};
...
test("every capability the page offers is still reachable after the fold", () => {
  const document = parseHtml(html);
  // Named per capability so a deletion fails by name rather than by count. Each
  // has its own behavioural test elsewhere; this holds the page to still
  // AUTHORING the surface those tests drive.
  const capabilities = {
    "provider detection": ["export-recognition", "provider-native-import"],
    "column mapping": ["import-mapping"],
    "retained periods": ["finops-track-record", "finops-retained-state"],
    "share links": ["finops-share", "finops-share-button", "finops-share-parity"],
    "rubric and fixture output": ["score-card", "recommendation-evidence"],
    "department drill-down": ["department-evidence", "finops-guided-department"],
  };
  for (const [capability, ids] of Object.entries(capabilities)) {
    for (const id of ids) {
      assert.ok(document.getElementById(id), `${capability} lost #${id} in the collapse`);
    }
  }
});
