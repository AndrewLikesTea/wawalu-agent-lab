...
//     lines sum to it is asserted over the bundled example and the real dataset
//     rather than assumed — and residual spend is a blocker in its own right, so
//     the sentence cannot ship if the invariant ever breaks.
//   * THE LITERACY JOIN IS PINNED. `literacy.departments[].departmentId` and
//     `rankedDepartments[].id` are two key spaces. A divergence would collapse
//     the metric to zero silently, which looks exactly like a real failure, so a
//     known department of the real dataset is asserted to read scored.
//
...
test("state complete and a 100% share are the same claim", () => {
  ...
  // The rounding case: 99.96% would print 100.0 without the clamp, over a
...
test("residual spend blocks complete, so the sentence cannot ship on an unproven sum", () => {
  ...
  // represents is a blocker of its own and complete is unreachable.
  const read = resolveRecoverableReadiness(signalsFor([line("a", 900)], 1000));
  assert.equal(read.state, READINESS_STATE.partial);
...
test("the adapter reads the analysis's own keys, and pins the literacy join", async () => {
  ...
  // THE JOIN. Literacy entries are keyed on departmentId and lines on id. If
  // those key spaces ever diverge every line reads unscored and the metric
  // collapses to zero, which is indistinguishable from a real coverage failure.
  assert.equal(analysis.literacy.available, true,
    "the example dataset no longer carries a graded query sample");
  const scored = signals.lines.filter((entry) => entry.scored);
  assert.ok(scored.length > 0,
    "no ranked line joined the literacy analysis: departmentId and id have diverged");
  const known = analysis.literacy.departments[0].departmentId;
  assert.equal(signals.lines.find((entry) => entry.id === known)?.scored, true,
    `the literacy-graded department ${known} does not read scored on its ranked line`);
