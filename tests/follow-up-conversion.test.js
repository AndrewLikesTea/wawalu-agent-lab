...
const CAPTURED_OPENING = /^Request received\. Your submitted work email was recorded\./;
const ALREADY_CAPTURED_OPENING = /^Request received\. That work email was already recorded/;
...
test("the AI FinOps confirmation begins with “Request received”, both times", async () => {
  const page = await openFinopsTab();
  const { document } = page;
...
test("a confirmed request keeps the briefing on screen and its print action within reach", async () => {
  ...
  try {
    ...
    // The confirmed fields, with no unguaranteed next-step promise.
