...
test("Coach, Releases, Social, People, and Agents send one bounded request and show durable success", async () => {
  ...
  for (const [file, requestType, pageContent] of pages) {
    ...
    try {
      ...
      if (topic) {
        assert.ok(textOf(confirmation).includes(`Fixed page topic: ${topic}.`),
      ...
      assert.match(textOf(confirmation), /may review the request and reply.*reply is not guaranteed/i,
        `${file}: confirmation gives a concrete, qualified next step`);
    } finally {
      page.restore();
    }
  }
...
test("the homepage footer sends and confirms its fixed page topic", async () => {
  for (const file of ["index.html"]) {
    const page = await openNamedFooterPage(file);
    const calls = interceptLeads(() => jsonReply({ captured: true, created: true, purpose: "follow_up_homepage" }, 201));
    try {
      assert.ok(!byId(page.document, "site-footer-topic"), `${file}: the fixed topic is prose, not an editable field`);
      submitEmail(page.document, "site-footer", LONG_EMAIL);
      await settled(page.document, "site-footer");

      assert.deepEqual(JSON.parse(calls[0].options.body), {
        email: LONG_EMAIL, purpose: "follow_up_homepage", topic: FOLLOW_UP_TOPICS.follow_up_homepage,
      }, `${file}: fixed topic reaches the wire`);
      const receipt = textOf(byId(page.document, "site-footer-confirmation"));
      assert.match(receipt, /Request received\. Submitted work email:/, `${file}: the receipt confirms receipt`);
      assert.match(receipt, new RegExp(FOLLOW_UP_TOPICS.follow_up_homepage));
    } finally {
      page.restore();
    }
  }
...
for (const { name, open, prefix, gated } of SURFACES) {
  test(`${name} answers a landed request with a receipt naming the address`, async () => {
    ...
    try {
      ...
      if (prefix === "site-footer") assert.match(textOf(receipt), /Fixed page topic:/i);
      else assert.doesNotMatch(textOf(receipt), /topic/i);
      assert.match(textOf(receipt), /may review the request and reply.*reply is not guaranteed/i);
      // And what stayed behind, named rather than left to be assumed: the
      // request carried one field, so the receipt says so out loud.
      assert.match(textOf(receipt), /Only that work email was entered by you and sent/);
