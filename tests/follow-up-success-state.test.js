...
test("Social, People, Releases, and Prompt coach all show the shared success confirmation", async () => {
  ...
  for (const file of pages) {
    ...
    try {
      ...
      assert.match(shownText(page.document, "site-footer-status"), /Wawalu will reply by email/,
...
for (const { name, open, prefix } of SURFACES) {
  test(`${name} answers a landed request with a receipt naming the address`, async () => {
    ...
    try {
      ...
      assert.match(textOf(receipt), /(?:A person from the Wawalu team replies|Wawalu will reply) to that address by email/);
      if (prefix === "site-footer") {
        assert.match(textOf(receipt), /Fields sent — work email: .*; stated interest: not provided\. No other fields were sent\./);
      } else {
  ...
  test(`${name} keeps its failure state, and a failure leaves no receipt`, async () => {
    ...
    try {
      ...
      assert.match(textOf(recovery), /Your email(?: address)?(?: and stated interest)? (?:is|are) still in the fields?/);
