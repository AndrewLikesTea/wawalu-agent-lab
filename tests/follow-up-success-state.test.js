...
for (const { name, open, prefix } of SURFACES) {
  test(`${name} answers a landed request with a receipt naming the address`, async () => {
    ...
    try {
      ...
      // Who answers, by when, and what travelled — the form's own vocabulary.
      // The reply window is the surface's own sentence repeated word for word,
      // not a second wording of it: see FOLLOW_UP_RESPONSE in lead-capture.js.
      ...
      // Nothing beyond the one window the team keeps.
      assert.doesNotMatch(textOf(receipt), /hours?\b|shortly|specialist|immediately/i);
