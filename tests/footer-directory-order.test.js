...
test("on every task page the follow-up block is read before the directory", async () => {
  for (const file of TASK_PAGES) {
    ...
    assert.equal(textOf(band[invitation]), file === "post.html"
      ? "Questions about this post from Social? Send the Wawalu team that operates Shiplog a follow-up request. The request does not include the post. Paste its link in your message so we know which post you mean."
