...
test("tabbing from the top reaches the picker, then the posts under the header", async () => {
  ...
  try {
    ...
    assert.equal(walked[5].classList.contains("profile-tile-link-label"), true, "the sixth stop is not the first post's Open post link");
    assert.equal(textOf(walked[5]), "Open post");
