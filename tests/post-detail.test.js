...
test("the loading state says the wait in words and reserves the post's five slots", () => {
  const container = createElement("div");
  ...
  assert.deepEqual(POST_SKELETON_SLOTS, ["image", "caption", "display-name", "timestamp", "actions"]);
  ...
  assert.equal(tags(container, "ARTICLE").length, 1, "the placeholder is shaped as a post article");
  assert.equal(tags(container, "FIGURE").length, 1, "the optional image keeps its figure relationship");
  assert.equal(tags(container, "FIGCAPTION").length, 1, "the caption reservation is semantic");
...
test("the shipped markup opens in the loading state, saying so in words", async () => {
  ...
  // this page's script has been fetched: the five slots, once each, in the order
  ...
  assert.deepEqual([...region.matchAll(/data-post-skeleton-slot="([a-z-]+)"/g)].map(([, slot]) => slot),
    ["image", "caption", "display-name", "timestamp", "actions"]);
  assert.equal((region.match(/class="detail-skeleton" aria-hidden="true"/g) ?? []).length, 1);
  assert.match(region, /<article class="detail-skeleton"[^>]*><figure class="detail-figure">/,
    "the placeholder uses the loaded post's article and figure hierarchy");
  assert.match(region, /<figcaption class="detail-caption">/,
    "the caption reservation is a caption, not an anonymous block");
