...
test("the post page introduces itself once, answering what a cold visitor cannot know", async () => {
  ...
  // The two feed waits name what is loading; Social's adds the next step. The
  // permalink keeps its own context-setting wait for a cold visitor. Both are
  // read as values rather than matched as literals in the source.
  //
  // They differ on purpose: Social's names its own composer trigger, which reads
  // "Write a post" (#2389), while People's names no next step at all, because
  // People has no composer to open and the paragraph that says where publishing
  // happens is off the page for the length of the fetch (#2416).
