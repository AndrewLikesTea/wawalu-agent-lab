...
import { commitSubjectOf, headCommitSha, stampSource, writeBuildStamp } from "../scripts/write-build-stamp.mjs";
...
test("the stamp is written from git, and a checkout without git is stamped unknown", async () => {
  ...
  assert.deepEqual(
    { ...stamped.BUILD_STAMP },
    { schemaVersion: 1, commitSha: sha, commitSubject: null, builtAt: "2026-08-06T09:15:00.000Z" },
  );
...
test("the stamp records the subject of the commit it names, verbatim, or null", async () => {
  const sha = "9f2c1ab4d7e60355a1c8b0d9e7f4a2b6c8d3e5f7";
  const subject = "Stop promising that visitor-published posts contain no customer or production data (#2310)";
  const calls = [];
  const git = async (...call) => {
    calls.push(call);
    return { stdout: `${subject}\n` };
  };
  assert.equal(await commitSubjectOf(sha, { git, cwd: "/checkout" }), subject);
  // The subject of the sha the stamp names, so the two fields name one commit.
  assert.deepEqual(calls, [["git", ["log", "-1", "--format=%s", sha], { cwd: "/checkout" }]]);

  // Only git's line terminator goes. Quotes, backslashes, a closing script tag
  // and trailing spaces survive into the generated module and back out of it.
  const awkward = 'Say "done" \\n, not \\\\ or `${x}` </script>  ';
  assert.equal(await commitSubjectOf(sha, { git: async () => ({ stdout: `${awkward}\n` }) }), awkward);
  const source = stampSource({ commitSha: sha, commitSubject: awkward, builtAt: "2026-08-06T09:15:00.000Z" });
  const stamped = await import(`data:text/javascript,${encodeURIComponent(source)}`);
  assert.equal(stamped.BUILD_STAMP.commitSubject, awkward);
  assert.equal(stamped.BUILD_STAMP.commitSha, sha);

  // Anything git cannot answer is no subject, never a placeholder.
  for (const failing of [
    async () => { throw new Error("git: command not found"); },
    async () => ({ stdout: "" }),
    async () => ({ stdout: "\n" }),
  ]) {
    assert.equal(await commitSubjectOf(sha, { git: failing }), null);
  }
  let asked = false;
  const eager = async () => {
    asked = true;
  ...
  assert.equal(await commitSubjectOf(null, { git: eager }), null);
  assert.equal(asked, false, "a build with no sha asked git for a subject anyway");
});
...
test("writeBuildStamp writes exactly one importable file and reports what it wrote", async (t) => {
  ...
  const subject = "Say why this build shipped (#2313)";

  const stamped = await writeBuildStamp({
    path,
    git: async (_command, args) => ({ stdout: args[0] === "rev-parse" ? `${sha}\n` : `${subject}\n` }),
    now: () => new Date("2026-08-06T09:15:00.000Z"),
  });
  assert.deepEqual(stamped, { commitSha: sha, commitSubject: subject, builtAt: "2026-08-06T09:15:00.000Z", path });
  const written = await import(`file://${path}`);
  assert.equal(written.BUILD_STAMP.commitSha, sha);
  assert.equal(written.BUILD_STAMP.commitSubject, subject);
  ...
  assert.deepEqual(unstamped, { commitSha: null, commitSubject: null, builtAt: null, path: unstampedPath });
