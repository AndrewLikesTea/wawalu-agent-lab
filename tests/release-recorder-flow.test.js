...
test("a release is recorded from the page with the decisions it carried", async (t) => {
  ...
  assert.equal(summaryText(page), "1 of 2 decisions linked. “Adopt a durable job queue” is the governing decision.");
...
test("more than one decision can be linked, in the order they were picked", async (t) => {
  ...
  assert.equal(summaryText(page), "2 of 2 decisions linked. “Adopt a durable job queue” is the governing decision.");
...
test("a rejected submit keeps the linked decisions and writes nothing", async (t) => {
  ...
  assert.equal(summaryText(page), "1 of 2 decisions linked. “Cache the read path” is the governing decision.");
...
test("a failed save is reported and preserves the release for retry", async (t) => {
  ...
  assert.equal(optionFor(page, QUEUE_DECISION.title).checked, true);
  assert.equal(summaryText(page), "1 of 1 decision linked. “Adopt a durable job queue” is the governing decision.");
