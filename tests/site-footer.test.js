...
const FOOTER_VARIANT = new Map([
  ...
  ["post.html", {
    ...
    invitation: 'Questions about this post from Social? Send the Wawalu team that operates Shiplog a follow-up request. The request does not include the post. Paste its link in your message so we know which post you mean.',
    messageHint: "Paste the post’s link, or its display name and a few of its words. Up to 200 characters.",
    collapsedDemos: true, askMessage: true, offer: true,
  }],
  ["releases.html", {
...
test("Social's homepage directory explains publishing, while a permalink explains the specific post", async () => {
  ...
  assert.ok(textOf(permalink).includes("The posts already on Social are invented to demonstrate Shiplog and use no customer or production data; a post a visitor publishes is real."));
...
test("the pending state is announced, not merely spun", async () => {
  ...
  try {
    ...
    const submit = byId(document, "site-footer-panel").querySelector('button[type="submit"]');
    assert.equal(submit.disabled, false, "aria-disabled preserves the focused control while the submit guard prevents duplicates");
