...
//
// The claim the copy makes is asserted against the
// outgoing request body, not against the sentence: the sentence is only true
// because `postLeadEmail` builds the whole body from explicit form fields.
...
const RECOVERY_COPY = "We could not send your follow-up request. Try again in a few minutes. "
  + "Your email and stated interest are still in the fields above.";
...
test("the footer form says what submitting asks for, on the page that carries both work-email forms", async () => {
  ...
  try {
    ...
    assert.equal(note, FOOTER_FOLLOW_UP_PRIVACY);
    // It names what is sent, who receives it, and that nothing else goes.
    assert.match(note, /work email and stated Shiplog interest/);
    assert.match(note, /Wawalu/);
...
test("a submission goes through the shared capture path, and the confirmation says what happens next", async () => {
  ...
  try {
    ...
    byId(document, "site-footer-interest").value = "Release decision trails";
    submitEmail(document, TYPED_EMAIL);
    await settled(document);

    // The shared path is /api/leads with a body built from the two visible fields.
    ...
    assert.deepEqual(JSON.parse(options.body), {
      email: TYPED_EMAIL, interest: "Release decision trails",
    });
    ...
    assert.match(confirmation, /Wawalu will reply by email/, "the confirmation must say what happens next");
    ...
    assert.match(receipt, /Wawalu will reply to that address by email/);
    assert.match(receipt, /work email: director@example\.com; stated interest: Release decision trails/);
    assert.doesNotMatch(receipt, /page activity|prompt|export|Social content/i);
...
test("the privacy sentence beside the field is what the request body actually does", async () => {
  ...
  try {
    assert.equal(shownText(document, "site-footer-note"), FOOTER_FOLLOW_UP_PRIVACY);
...
test("a failed submission keeps the typed address, says it can be retried, and the retry works", async () => {
  ...
  try {
    ...
    interest.value = "How releases connect to decisions";
    const submit = byId(document, "site-footer-panel").querySelector('button[type="submit"]');
    ...
    assert.equal(interest.value, "How releases connect to decisions", "a failed submission must preserve stated interest");
    ...
    assert.deepEqual(JSON.parse(calls[1].options.body), {
      email: TYPED_EMAIL, interest: "How releases connect to decisions",
    });
    assert.match(shownText(document, "site-footer-status"), /^Follow-up requested\./);
  } finally {
    page.restore();
  }
});
