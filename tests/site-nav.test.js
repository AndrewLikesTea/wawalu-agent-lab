...
test("the profile page defines the selected name as a display name", async () => {
  ...
  assert.match(role[1], /^The posts already on Social carry invented display names\. On any other post, whoever published it chose the name\./);
  ...
  assert.match(role[1], /Nobody owns or verifies a display name, so anyone can publish under any name/);
...
test("Social and People lead with the same one-line, verb-first tagline the rest of the site does", async () => {
  ...
  assert.match(coach,
    /<h1 id="page-title">Prompt coach<\/h1>\s*<p id="page-tagline">Grade a prompt before you send it\./,
    "Prompt coach no longer names itself and then says what you do here");
