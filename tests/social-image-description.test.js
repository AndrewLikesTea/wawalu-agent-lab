...
test("the description requirement is stated only once there is an image to describe", async (t) => {
  ...
  // The marker names the condition, because the condition is what the code
  // enforces: a caption-only post publishes with this field empty.
...
test("the shipped composer markup carries the marker, the error slot, and the counter", async () => {
  ...
  // Stated in the page as served, and stated with its condition: the field is
  // required once an image is attached and not otherwise, so the served label
  // answers "must I fill this in?" truthfully in both states rather than
  // claiming a requirement a caption-only post never meets. social.js still
