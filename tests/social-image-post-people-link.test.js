...
test("choosing a name on People writes it into the address, and that address reopens on it", async (t) => {
  ...
  // A reload is a cold open of the address the page wrote: nothing remembered,
  // so only the URL can put Ari back.
...
const AWKWARD_FEED = ["Ana María", "A & B", "x#y?z", "<b>Zed"].map((author, index) => ({
  ...post(`p-1${index}`, author, `1${index}`),
  image: { src: `/media/awkward-${index}.svg`, alt: `Drawing ${index}`, width: 1200, height: 900 },
}));

test("names that need encoding or look like markup reach their own People view, read literally", async (t) => {
  const social = await openSocial(t, { feed: AWKWARD_FEED });
  const hrefs = AWKWARD_FEED.map((entry) => {
    const card = drawnCards(social.document).find((node) => node.dataset?.postId === entry.id);
    ...
    assert.equal(textOf(link), `See ${entry.author}’s image posts on People`);
    const href = link.getAttribute("href");
    assert.equal(new URLSearchParams(new URL(href, "https://labs.wawalu.org").search).get("author"), entry.author,
      `the address does not carry ${entry.author} whole`);
    return href;
  });

  for (const [index, href] of hrefs.entries()) {
    const { author, id } = AWKWARD_FEED[index];
    const { document } = await openPeople(t, { feed: AWKWARD_FEED, search: new URL(href, "https://labs.wawalu.org").search });
...
// The harness parses no markup, so a name like "<b>Zed" cannot fail the test
