...
const CLEARED = "The attempted selection was cleared; any prior valid image was also removed.";
const UNSUPPORTED_TYPE = `This file is not a PNG, JPEG, GIF, or WebP. ${CLEARED} Convert or re-export it in an accepted format, then choose it again.`;
const OVER_LIMIT = `This file is 513 KB; the maximum is 512 KB. ${CLEARED} Pick a smaller file, or export a smaller PNG from Paint.`;
...
test("a file in an unaccepted format is named as such, with the four accepted formats", async (t) => {
  ...
  assert.equal(textOf(document.querySelector("#post-publish-blocker")), `Publishing is blocked: ${UNSUPPORTED_TYPE}`);
  ...
  assert.equal(document.querySelectorAll("#post-media-status[aria-live]").length, 1,
    "the refusal is announced by exactly one live region");
  assert.equal(document.querySelector("#post-image-steps").querySelector("a")?.getAttribute("href"), "/paint/",
    "the refusal is immediately followed by the Paint recovery link");
});
...
test("a file over the limit is told the limit, in the words the field's help text uses", async (t) => {
  ...
  assert.equal(overLimitError(512 * 1024 + 1), OVER_LIMIT);
  assert.equal(status.classList.contains("is-error"), true);
  assert.equal(document.querySelector("#compose-media").hidden, true);
  assert.equal(document.querySelector("#post-image").value, "");
  assert.equal(document.querySelector("#post-submit").disabled, true);
  assert.equal(textOf(document.querySelector("#post-publish-blocker")), `Publishing is blocked: ${OVER_LIMIT}`);
  assert.equal(document.querySelector("#post-publish-blocker").getAttribute("aria-live"), null);
  assert.equal(document.querySelectorAll("#post-media-status[aria-live]").length, 1,
    "the refusal is announced by exactly one live region");
  assert.equal(document.querySelector("#post-image-steps").querySelector("a")?.getAttribute("href"), "/paint/");
...
test("the help text and the refusals state the limit and the formats identically", async () => {
  ...
  const source = `${markup}\n${wiring}\n${copy}`;
  assert.match(markup,
    /id="post-image-steps"><a[^>]*>Create an image in Paint[\s\S]*PNG no larger than 512 KB[\s\S]*too large, resize or simplify/);
