...
test("the forwarded People URL opens cold on that display name, filtered and newest first", async (t) => {
  ...
  assert.equal(textOf(document.querySelector("#profile-name")), `People is filtered to ${REMY}’s image posts.`);
...
test("a forwarded name with no image posts is named, and offered the whole feed", async (t) => {
  ...
  assert.equal(textOf(document.querySelector("#profile-name")), `People is filtered to ${TESS}’s image posts.`);
  ...
  assert.match(textOf(document.querySelector("#profile-announcer")), /^Tess Nakano hasn’t posted an image yet\./);
});
