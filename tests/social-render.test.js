...
test("an image post renders as a figure with the caption as its figcaption", () => {
  ...
  assert.equal(author.textContent, "See Mina’s image posts on People");
  assert.equal(author.getAttribute("aria-label"), null);
  assert.equal(first(card, "post-name").textContent, "Mina");
