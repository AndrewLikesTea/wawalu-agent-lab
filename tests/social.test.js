...
test("social page is wired, labeled, and linked from the other pages", async () => {
  ...
  // One announced region for a filter change, and it is the heading itself: the
  // count slot beside it and the summary paragraph that used to sit under the
  // filters both said a version of the same news, so announcing more than one
  // read every change out twice.
  assert.match(page, /<h2 id="feed-title" tabindex="-1" aria-live="polite" aria-atomic="true">All posts<\/h2>/);
  assert.doesNotMatch(page, /id="post-count"[^>]*aria-live/);
  // The second visible copy of the count is gone from the markup entirely, rule
  // and all — not merely emptied, which would leave a node for a later change to
  // start writing to again.
  assert.doesNotMatch(page, /feed-summary/);
  assert.doesNotMatch(styles, /\.feed-summary/);
  // The live region ships unhidden and not counting: a `hidden` region announces
  // unreliably when its text arrives, and a count before the first fetch is a
...
test("the feed toolbar names what each control filters, in the site's own terms", async (t) => {
  ...
  // the feed heading (asserted on a booted page below).
...
// is on screen now, out of how many the feed holds, in the same words the two
// menus use — "author" would be a second word for something this site already
// names, and the composer's field hint, People's picker, this filter, and the
// note under Clear filters all say "display name".
test("feedHeading counts what the filters are showing, in the menus' own words", () => {
  assert.equal(AUTHOR_TERM, "display name");
  assert.equal(DEFAULT_FEED_HEADING, "All posts");

  // No filter: the total, and no denominator, because there is nothing the feed
  // has been narrowed from.
  assert.equal(feedHeading({ shown: 12, total: 12 }), "All 12 posts");
  // Nothing to be "all" of: one post is the whole feed, and "All 1 post" is not
  // a phrase anyone writes.
  assert.equal(feedHeading({ shown: 1, total: 1 }), "1 post");
  ...
  assert.equal(feedHeading({ shown: 0, total: 0 }), "No posts");
  // `total` defaults to `shown`, so an unfiltered caller need not repeat itself.
  assert.equal(feedHeading({ shown: 12 }), "All 12 posts");

  // Display name only, and a narrowed feed says what it was narrowed *from*:
  // without the denominator, "3 posts" and "All 3 posts" are the same phrase to
  // a reader who cannot see the menus above it.
  assert.equal(feedHeading({ shown: 3, total: 12, author: "Ari" }),
    `3 of 12 posts under the ${AUTHOR_TERM} Ari`);
  // Time only, in the time menu's own option text.
  assert.equal(feedHeading({ shown: 5, total: 12, range: "from the past 7 days" }),
    "5 of 12 posts from the past 7 days");
  // Both, named in one phrase carrying one count.
  assert.equal(feedHeading({ shown: 2, total: 12, range: "from the past 7 days", author: "Ari" }),
    "2 of 12 posts from the past 7 days under the display name Ari");

  // The plural follows the total, which is the noun being counted out of.
  assert.equal(feedHeading({ shown: 1, total: 1, author: "Mina" }),
    "1 of 1 post under the display name Mina");
  ...
  assert.equal(feedHeading({ shown: 0, total: 9, author: "Ari" }),
    "0 of 9 posts under the display name Ari");
  assert.equal(feedHeading({ shown: 0, total: 9, range: "from the past hour", author: "Ari" }),
    "0 of 9 posts from the past hour under the display name Ari");

  // Never the fixed wording while a filter is on, in any of the three shapes.
  for (const filters of [{ author: "Ari" }, { range: "from the past hour" },
    { range: "from the past hour", author: "Ari" }]) {
    for (const shown of [0, 1, 5]) {
      const text = feedHeading({ shown, total: 9, ...filters });
      assert.notEqual(text, DEFAULT_FEED_HEADING, "a filtered feed still claims to hold every post");
      assert.doesNotMatch(text, /^All /, "a filtered feed still claims to hold every post");
      // A heading, not the sentence that used to sit under the filters.
...
test("the feed heading tracks the filters and the cards actually rendered", async (t) => {
  ...
  // The first-load wording, kept as what the builder produces rather than as a
  // literal, so the round-trip below compares the page against the one source of
  // this string instead of against a second copy of it.
  const unfiltered = feedHeading({ shown: 3, total: 3 });
  assert.equal(textOf(heading), unfiltered);
  assert.equal(shown(), 3);

  // The sentence is on the page once. Counted by matching text over every
  // element rather than by asking one element what it says: a second node
  // repeating it — the summary paragraph that used to sit under the filters, or
  // the count slot beside the heading — is exactly what this has to catch, and
  // naming the elements to check would be assuming the answer.
  const saying = (text) => {
    let count = 0;
    const visit = (node) => {
      for (const child of node.children) {
        if (child.nodeType !== 1) continue;
        if (textOf(child) === text) count += 1;
        visit(child);
      }
    };
    visit(page.document);
  ...
  assert.equal(saying(unfiltered), 1, "the sentence is rendered in exactly one place");

  choose(nameFilter, "Ari");
  assert.equal(textOf(heading), "2 of 3 posts under the display name Ari");
  assert.equal(shown(), 2, "the heading's count is the number of cards rendered");
  assert.equal(saying(textOf(heading)), 1, "a filter change updates one place, not two");

  choose(nameFilter, "all");
  choose(timeFilter, "hour");
  assert.equal(textOf(heading), "1 of 3 posts from the past hour");
  assert.equal(shown(), 1);

  choose(nameFilter, "Ari");
  assert.equal(textOf(heading), "1 of 3 posts from the past hour under the display name Ari",
    "both filters are named, and the pair carries one count");
  assert.equal(shown(), 1);

  choose(nameFilter, "Mina");
  assert.equal(textOf(heading), "0 of 3 posts from the past hour under the display name Mina");
  ...
  assert.equal(textOf(heading), unfiltered, "Clear filters restores the first-load wording exactly");
  assert.equal(shown(), 3);
  assert.equal(saying(unfiltered), 1);
  ...
  // The heading is the one announced region: the same string a reader sees is
  // the string a screen reader hears on a filter change, and it is announced
  // whole. A second live region — on the count beside it, or on a paragraph
  // repeating the count below — would read the same news twice.
  assert.equal(heading.getAttribute("aria-live"), "polite");
  assert.equal(heading.getAttribute("aria-atomic"), "true");
  assert.equal(heading.getAttribute("role"), null, "a role=status on the same text announces it twice");
  assert.equal(heading.querySelectorAll("[aria-live]").length, 0, "a nested live region announces twice");
  assert.equal(page.document.querySelector("#post-count").getAttribute("aria-live"), null);
  assert.deepEqual(panel.querySelectorAll("[aria-live]").map((node) => node.id || node.className),
    ["feed-title", "feed-connection", "feed-announcer"],
    "the panel announces from the heading, the connection line, and the new-posts announcer, and nowhere else");
});
...
test("a filter combination matching nothing reads as a dead end with its own recovery", async (t) => {
  ...
  assert.equal(textOf(page.document.querySelector("#feed-title")), feedHeading({ shown: 3, total: 3 }),
    "recovering from the dead end restores the first-load wording, not a second phrasing of it");
...
// The live region has to persist across updates, so the element ships with the
// page rather than being created when there is finally something to say. Never
// hidden, and never emptied: a `hidden` region announces unreliably when its
// text arrives, and this one is also the panel's accessible name.
test("the announced heading persists across every state of the feed", async (t) => {
  const page = await loadPage(new URL("../src/social.html", import.meta.url), {});
  t.after(() => page.restore());
  const heading = page.document.querySelector("#feed-title");
  const feed = mountSocialFeed(page.document, { posts: [], state: "loading" });

  for (const step of [() => {}, () => feed.setState("error"), () => feed.seed([])]) {
    step();
    assert.equal(heading.getAttribute("aria-live"), "polite");
    assert.equal(heading.hasAttribute("hidden"), false);
    assert.notEqual(textOf(heading), "", "the panel's accessible name is never blanked");
    assert.equal(page.document.querySelectorAll("#feed-title").length, 1,
      "the region is replaced text, not a replaced element — a new node announces unreliably");
  }
  assert.match(textOf(page.document.querySelector(".empty-state")), /No posts on Social yet\./);
});

// The shipped markup only pins the word a visitor sees before the feed mounts.
...
test("the post count slot never claims zero posts before the feed has any answer", async (t) => {
  const page = await loadPage(new URL("../src/social.html", import.meta.url), {});
  t.after(() => page.restore());
  ...
  feed.seed([]);
  // Once the feed can count itself, this slot stops counting: the heading beside
  // it says "No posts", and a second copy of that number on the same line is the
  // duplication the heading exists to end.
  assert.equal(textOf(count), "", "the answered count belongs to the heading, and is said once");
  assert.equal(textOf(page.document.querySelector("#feed-title")), "No posts",
    "an answered fetch with nothing in it is a real zero");
